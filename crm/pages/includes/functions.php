<?php
	function get_notifications($username, $type) {
		switch($type) {
			...
			case 'new_user_registered' : 
				...
				return $new_tickets->num_rows; break;
			case 'new_payment' : 
				$payments = get_some_data("payments", "remark = '0'");
	...
	function user_at_a_glance1($index) {
		//$user_info = get_user_info($username);
		$user_orders = get_order_history1();
		$user_orders_paid = get_order_history1("status = '3'");
		$user_orders_rm_placed = get_order_history1("status != '1'");
	...
	function get_all_lead($tablename, $extra_sql=true) {
		global $conn;
		$get = "SELECT * FROM {$tablename} ";
		$get.= $extra_sql; 
		$result = $conn->query($get) or trigger_error($get);
